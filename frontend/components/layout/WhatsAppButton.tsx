...
export function WhatsAppButton({ contact }: { contact?: ContactSettings }) {
  const [hovered, setHovered] = useState(false);

  // Clean the number of non-numeric characters; default to a fallback if none configured
  const rawNumber = contact?.whatsapp || contact?.phone || "";
  const cleanNumber = rawNumber.replace(/\D/g, "");

  const defaultMessage = encodeURIComponent(
    "Hello JULES & CO, I would like to make an inquiry about your pieces."
  );
